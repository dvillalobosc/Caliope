...
QString StaticFunctions::serverInformationQuery()
{
  ...
  case StaticFunctions::MySQL:
  case StaticFunctions::MariaDB:
    return "SELECT '" + tr("Uptime in days") + "' AS `" + tr("Variable") + "`,  LPAD(FORMAT(`VARIABLE_VALUE` / 60 / 60 / 24, 2), 12, ' ') AS `" + tr("Value") + "`, '" + tr("Number of days the server has been running.") + "' AS `" + tr("Description") + "`, 'UPTIME' AS `" + tr("Variable mame") + "` FROM `information_schema`.`GLOBAL_STATUS` WHERE `VARIABLE_NAME` = 'UPTIME'"
        " UNION"
        " SELECT '" + tr("Aborted clients") + "', LPAD(FORMAT(`VARIABLE_VALUE`, 0), 12, ' '), '" + tr("Number of aborted client connections.") + "', 'ABORTED_CLIENTS' FROM `information_schema`.`GLOBAL_STATUS` WHERE `VARIABLE_NAME` = 'ABORTED_CLIENTS'"
        " UNION"
        ...
        " UNION"
        " SELECT '" + tr("Aborted connections") + "', LPAD(`VARIABLE_VALUE`, 12, ' '), '" + tr("Number of failed server connection attempts.") + "', 'ABORTED_CONNECTS' FROM `information_schema`.`GLOBAL_STATUS` WHERE `VARIABLE_NAME` = 'ABORTED_CONNECTS'"
        " UNION"
        " SELECT '" + tr("Executed rollbacks") + "', LPAD(FORMAT(`VARIABLE_VALUE`, 0), 12, ' '), '" + tr("Number of ROLLBACK commands executed.") + "', 'COM_ROLLBACK' FROM `information_schema`.`GLOBAL_STATUS` WHERE `VARIABLE_NAME` = 'COM_ROLLBACK'"
        " UNION"
        ...
        " UNION"
        " SELECT '" + tr("Executed queries") + "', LPAD(FORMAT(`VARIABLE_VALUE`, 0), 12, ' '), '" + tr("Number of statements executed by the server.") + "', 'QUERIES' FROM `information_schema`.`GLOBAL_STATUS` WHERE `VARIABLE_NAME` = 'QUERIES'"
        " UNION"
        " SELECT '" + tr("Temporary disk tables created") + "', LPAD(FORMAT(`VARIABLE_VALUE`, 0), 12, ' '), '" + tr("Number of on-disk temporary tables created.") + "', 'CREATED_TMP_DISK_TABLES' FROM `information_schema`.`GLOBAL_STATUS` WHERE `VARIABLE_NAME` = 'CREATED_TMP_DISK_TABLES'"
        " UNION"
        " SELECT '" + tr("Free cache memory") + "', LPAD(FORMAT(`VARIABLE_VALUE` / 1024, 0), 12, ' '), '" + tr("Amount of free query cache memory.") + "', 'QCACHE_FREE_MEMORY' FROM `information_schema`.`GLOBAL_STATUS` WHERE `VARIABLE_NAME` = 'QCACHE_FREE_MEMORY'"
        " UNION"
        " SELECT '" + tr("Joins with full table scan") + "', LPAD(FORMAT(`VARIABLE_VALUE`, 0), 12, ' '), '" + tr("Number of joins which did not use an index. If not zero, you may need to check table indexes.") + "', 'SELECT_FULL_JOIN' FROM `information_schema`.`GLOBAL_STATUS` WHERE `VARIABLE_NAME` = 'SELECT_FULL_JOIN'"
        " UNION"
        " SELECT '" + tr("Log slow queries") + "', LPAD(`VARIABLE_VALUE`, 12, ' '), '" + tr("Is the slow queries logging enabled?") + "', 'LOG_SLOW_QUERIES' FROM `information_schema`.`GLOBAL_VARIABLES` WHERE `VARIABLE_NAME` = 'SLOW_QUERY_LOG'"
        " UNION"
        " SELECT '" + tr("Slow queries time in seconds") + "', LPAD(FORMAT(`VARIABLE_VALUE`, 0), 12, ' '), '" + tr("The number of seconds that determinate a slow query.") + "', 'LONG_QUERY_TIME' FROM `information_schema`.`GLOBAL_VARIABLES` WHERE `VARIABLE_NAME` = 'LONG_QUERY_TIME'"
        " UNION"
        " SELECT '" + tr("Count of slow queries") + "', LPAD(FORMAT(`VARIABLE_VALUE`, 0), 12, ' '), '" + tr("Number of queries which took longer than long_query_time to run.") + "', 'SLOW_QUERIES' FROM `information_schema`.`GLOBAL_STATUS` WHERE `VARIABLE_NAME` = 'SLOW_QUERIES'"
        " UNION"
        ...
        " UNION"
        " SELECT '" + tr("Max connections aviable") + "', LPAD(FORMAT(`VARIABLE_VALUE`, 0), 12, ' '), '" + tr("The maximum number of simultaneous client connections.") + "', 'MAX_CONNECTIONS' FROM `information_schema`.`GLOBAL_VARIABLES` WHERE `VARIABLE_NAME` = 'MAX_CONNECTIONS'"
        " UNION"
